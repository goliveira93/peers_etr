...
def gera_df_performance(fundo: str, save_files: bool = True) -> pd.DataFrame:
    ...
    query = "SELECT DT_COMPTC, CNPJ_FUNDO_COTA, NM_FUNDO_COTA FROM " + table[0] + " WHERE DT_COMPTC = '2024-01-31 00:00:00' LIMIT 2"
    df = pd.read_sql(query, con=engine)

    query2 = "SELECT Gestor, DT_COMPTC, NM_FUNDO_COTA FROM " + table[0] + " WHERE DT_COMPTC = '2024-01-31 00:00:00' LIMIT 2"
    ...
    #if table[1] == "EON":
        # Se a tabela for FIM (EON), adiciona IFMM e CDI
    for benchmark in table[1]:
        nova_linha = {"DT_COMPTC": df2.iloc[1]["DT_COMPTC"], "NM_FUNDO_COTA": benchmark, "CNPJ_FUNDO_COTA":benchmark}
        #nova_linha_cdi1 = {"DT_COMPTC": df2.iloc[1]["DT_COMPTC"], "NM_FUNDO_COTA": "CDI", "CNPJ_FUNDO_COTA":"CDI"}
    ...
    # Atualizando o layout do gráfico
        ...
        fig.write_image(os.path.join(diretorio_base, f"{table[2]}_retorno_mtd.png"))


        print(f"gerando {table[2]} mtd")
        # Criando o gráfico de barras
        fig = go.Figure(go.Bar(
        ...
        # Exibindo o gráfico
        print(f"gerando {table[2]} ytd")
        fig.write_image(os.path.join(diretorio_base, f"{table[2]}_retorno_ytd.png"))
